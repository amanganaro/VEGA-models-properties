...
public class mainScriptMoaEpa {
    private static final Logger log = LogManager.getLogger(mainScriptMoaEpa.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismMoaEpa();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-MoaEpa\\src\\main\\resources\\data\\ts_moa_epa.dat");
        try {
