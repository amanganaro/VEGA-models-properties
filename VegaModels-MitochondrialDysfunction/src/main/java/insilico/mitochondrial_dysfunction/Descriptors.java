...
public class Descriptors {
    ...
    private final MitochondrialDysfunction mitochondrialDysfunction;
    private Path pathToExternalFolder;

    public Descriptors(MitochondrialDysfunction model) {
        mitochondrialDysfunction = model;
        ...
        if (System.getProperty("os.name").startsWith("Windows")) {
            pathToExternalFolder = Paths.get(System.getProperty("user.home"),
                    "\\AppData\\Local\\vega-models\\mitochondrial-dysfunction\\python\\").resolve("");
        }
        else {
            pathToExternalFolder = Paths.get(System.getProperty("user.home") ,"/.local/share/vega-models/mitochondrial-dysfunction/python/");
        }
    }

    public boolean calculateDescriptors() throws IOException, InterruptedException, URISyntaxException {
        boolean isEnvSet = mitochondrialDysfunction.CHECK_SETUP ? configureCondaEnv() : true;
        boolean result = false;
        if (isEnvSet) {
            log.info("Start to calculate descriptors");
            result = communication.executeCommandInCondaEnv("cddd", "cddd",
                    "--input "+ mitochondrialDysfunction.getInputTempFile(), " --output "+ mitochondrialDysfunction.getDescriptorsTempFile(),
                    " --smiles_header smiles");
            File f = new File(mitochondrialDysfunction.getInputTempFile());
            f.delete();
