...
// note: function svm_load_model has been modified from original libsvm
// in order to accept an InputStream as parameter

package insilico.mutagenicity_caesar.libsvm;
...
class Cache {
	...
	int get_data(int index, float[][] data, int len)
	{
		...
		if(more > 0)
		{
			...
			h.data = new_data;
			size -= more;
			do {int buff=h.len; h.len=len; len=buff;} while(false);
		}
	...
	void swap_index(int i, int j)
	{
		...
		do {float[] buff=head[i].data; head[i].data=head[j].data; head[j].data=buff;} while(false);
		do {int buff=head[i].len; head[i].len=head[j].len; head[j].len=buff;} while(false);
		if(head[i].len > 0) lru_insert(head[i]);
		if(head[j].len > 0) lru_insert(head[j]);

		if(i>j) do {int buff=i; i=j; j=buff;} while(false);
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {float buff=h.data[i]; h.data[i]=h.data[j]; h.data[j]=buff;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
abstract class Kernel extends QMatrix {
	...
	void swap_index(int i, int j)
	{
		do {svm_node[] buff=x[i]; x[i]=x[j]; x[j]=buff;} while(false);
		if(x_square != null) do {double buff=x_square[i]; x_square[i]=x_square[j]; x_square[j]=buff;} while(false);
	}
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte buff=y[i]; y[i]=y[j]; y[j]=buff;} while(false);
		do {double buff=G[i]; G[i]=G[j]; G[j]=buff;} while(false);
		do {byte buff=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=buff;} while(false);
		do {double buff=alpha[i]; alpha[i]=alpha[j]; alpha[j]=buff;} while(false);
		do {double buff=p[i]; p[i]=p[j]; p[j]=buff;} while(false);
		do {int buff=active_set[i]; active_set[i]=active_set[j]; active_set[j]=buff;} while(false);
		do {double buff=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=buff;} while(false);
	}
...
class SVC_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		...
		do {byte buff=y[i]; y[i]=y[j]; y[j]=buff;} while(false);
		do {float buff=QD[i]; QD[i]=QD[j]; QD[j]=buff;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		do {byte buff=sign[i]; sign[i]=sign[j]; sign[j]=buff;} while(false);
		do {int buff=index[i]; index[i]=index[j]; index[j]=buff;} while(false);
		do {float buff=QD[i]; QD[i]=QD[j]; QD[j]=buff;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int buff=perm[i]; perm[i]=perm[j]; perm[j]=buff;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
	...
	public static void svm_cross_validation(svm_problem prob, svm_parameter param, int nr_fold, double[] target)
	{
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int buff=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=buff;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
		...
		else
		{
			...
			for(i=0;i<l;i++)
			{
				...
				do {
					int buff=perm[i]; perm[i]=perm[j]; perm[j]=buff;
				} while(false);
			}
			for(i=0;i<=nr_fold;i++)
	...
	public static svm_model svm_load_model(InputStream model_file_name) throws IOException
	{
		BufferedReader fp = new BufferedReader(new InputStreamReader(model_file_name));
