...
public class mainScriptMicroVitro {
    private static final Logger log = LogManager.getLogger(mainScriptMicroVitro.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismMicronucleusInVitro();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-MicronucleusVitro\\src\\main\\resources\\data\\ts_micronucleus_vitro.dat");
        try {
