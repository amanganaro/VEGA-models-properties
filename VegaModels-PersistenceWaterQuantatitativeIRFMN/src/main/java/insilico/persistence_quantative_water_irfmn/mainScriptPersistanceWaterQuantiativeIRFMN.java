...
public class mainScriptPersistanceWaterQuantiativeIRFMN {
    private static final Logger log = LogManager.getLogger(mainScriptPersistanceWaterQuantiativeIRFMN.class);

    public static void main(String[] args) throws Exception {

        InsilicoModel model = new ismPersistenceWaterQuantitativeIrfmn();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-PersistenceWaterQuantatitativeIRFMN\\src\\main\\resources\\data\\ts_water_quantitative_irfmn.dat");
        try {
