...
public class mainScriptNoaelCoral {
    private static final Logger log = LogManager.getLogger(mainScriptNoaelCoral.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismNoaelCoral();
        model.setSkipADandTSLoading(false);
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-NoaelCoral\\src\\main\\resources\\data\\ts_noael_coral.dat");
        try {
