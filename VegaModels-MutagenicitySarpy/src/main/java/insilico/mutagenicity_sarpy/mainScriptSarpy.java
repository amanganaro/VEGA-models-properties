...
public class mainScriptSarpy {
    private static final Logger log = LogManager.getLogger(mainScriptSarpy.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismMutagenicitySarpy();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-MutagenicitySarpy\\src\\main\\resources\\data\\ts_muta_sarpy.dat");
        try {
