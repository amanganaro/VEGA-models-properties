package insilico.mutagenicity_sarpy;
...
public class ismMutagenicitySarpy extends InsilicoModel {
    private static final Logger log = LogManager.getLogger(ismMutagenicitySarpy.class);

    private static final long serialVersionUID = 1L;
    
    private static final String ModelData = "/data/model_muta_sarpy.xml";
    
    private final String SarpyFullAlertSet;
    
    public ismMutagenicitySarpy() 
            throws InitFailureException {
        super(ModelData);
        
        // Set SA list
        SarpyFullAlertSet = AlertEncoding.MergeAlertIds((new SAMutagenSarpy()).getAlerts());
        ...
        this.ResultsName[1] = "No. alerts for mutagenicity";
        this.ResultsName[2] = "No. alerts for non-mutagenicity";
    ...
    public ArrayList<Integer> GetRequiredAlertBlocks() {
        ...
        blocks.add(InsilicoConstants.SA_BLOCK_MUTAGEN_SARPY);
        return blocks;
    }    
    ...
    protected short CalculateModel() {
        ...
        for (Alert a : CurMolecule.GetAlerts().getSAList())
            if (AlertEncoding.ContainsAlert(SarpyFullAlertSet, a.getId())) {
                if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_IS_TOXIC))
                    nMutagen++;
                else
                    nNonMutagen++;
            }
        ...
        Res[1] = Integer.toString(nMutagen);
        Res[2] = Integer.toString(nNonMutagen);
