 ...
 * @author User
 */
public class ismDaphniaCombase extends InsilicoModel {
    
    private static final long serialVersionUID = 1L;
    
    private static final String ModelData = "/data/model_daphnia_combase.xml";

    private CoralModel DaphniaModel;
    private double MW;
    
        
    public ismDaphniaCombase() 
            throws InitFailureException {
        super(ModelData);
        
        // Init Coral model
        try {  
            DaphniaModel = new OptimizedCoralDaphnia();
        } catch (Exception ex) {
    ...
    protected short CalculateModel() {
        ...
        double Prediction;
        try {
            Prediction = DaphniaModel.Predict(this.CurMolecule.GetSMILES());
        } catch (Exception ex) {
            return MODEL_ERROR;
        }
