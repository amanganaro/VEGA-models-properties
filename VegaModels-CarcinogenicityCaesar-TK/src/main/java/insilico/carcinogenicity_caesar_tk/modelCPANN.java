package insilico.carcinogenicity_caesar_tk;
...
public class modelCPANN {
	...
	int n=0, j=0, k=0;
        ...
        u = getClass().getResource("/data/carccaesar_nn_weights.csv");
	...
	int n=0, j=0, k=0;
        ...
        u = getClass().getResource("/data/carccaesar_nn_normalization.csv");
        InputStream is = u.openStream();
        BufferedReader br = new BufferedReader(new InputStreamReader(is));
    ...
    public double[] CalculatePrediction(double[] Descriptors) {
        ...
        double val;
        int MinX=0, MinY=0;
        double[] Res = new double[4];   // Response: Positive/Non-Positive +
                                        // NeuronX, NeuronY
        ...
        Res[1] = NN[MinX][MinY].Out_Layer[1]; // Non-Positive
        Res[2] = MinX;
        Res[3] = MinY;
