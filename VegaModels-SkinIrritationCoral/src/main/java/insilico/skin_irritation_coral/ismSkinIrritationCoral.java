...
import insilico.core.coral.models.skin.irritation.CoralSkinIrritation;
...
public class ismSkinIrritationCoral extends InsilicoModel {
    ...
    private static final Logger log = LogManager.getLogger(ismSkinIrritationCoral.class);

    private static final String ModelData = "/data/model_skin_irritation_coral.xml";

    private CoralModel SkinIrrCoral;
    ...
    public ismSkinIrritationCoral() throws InitFailureException {
        ...
        // Init Coral model
        try {
            SkinIrrCoral = new CoralSkinIrritation();
        } catch (Exception ex) {
        ...
        this.ResultsName[0] = "Skin irritation activity";
    ...
    protected short CalculateModel() {
        ...
        double Prediction;
        try {
            Prediction = SkinIrrCoral.Predict(this.CurMolecule.GetSMILES());
        } catch (Exception ex) {
            return MODEL_ERROR;
        }

        int PredClass;
        if (Prediction < 0) PredClass = 0;
        else PredClass = 1;
