...
public class mainScriptFishIRFMN {

    private static final Logger log = LogManager.getLogger(mainScriptFishIRFMN.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        InsilicoModel model = new ismFishIRFMN();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-FishIRFMN\\src\\main\\resources\\data\\ts_fish_irfmn.dat");
        try {
