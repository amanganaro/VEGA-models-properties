...
public class ismLoaelCoralLiver extends InsilicoModel {
    private static final Logger log = LogManager.getLogger(ismLoaelCoralLiver.class);

    private static final long serialVersionUID = 1L;

    private static final String ModelData = "/data/model_loael_liver_coral.xml";

    private CoralModel LoaelLiver;
    ...
    protected short CalculateModel() {
        double Prediction;
        try {
            Prediction = LoaelLiver.Predict(this.CurMolecule.GetSMILES());
        } catch (Exception ex) {
            return MODEL_ERROR;
        }
