...
public class mainScriptPersSedimentQuantitative {

    private static final Logger log = LogManager.getLogger(mainScriptPersSedimentQuantitative.class);


    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismPersistenceSedimentQuantitativeIrfmn();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-PersistenceSedimentQuantitativeIRFMN\\src\\main\\resources\\data\\ts_sediment_quantitative_irfmn.dat");
        try {
