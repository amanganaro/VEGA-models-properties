...
public class mainScriptNoaelGeneralCoral {

    private static final Logger log = LogManager.getLogger(mainScriptNoaelGeneralCoral.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismNoaelGeneralCoral();
        model.setSkipADandTSLoading(false);
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-NoaelGeneralCoral\\src\\main\\resources\\data\\ts_noael_general_coral.dat");
        try {
