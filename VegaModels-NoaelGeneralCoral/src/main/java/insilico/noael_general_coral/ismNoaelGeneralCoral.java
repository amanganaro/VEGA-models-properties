package insilico.noael_general_coral;
...
public class ismNoaelGeneralCoral extends InsilicoModel {

    private static final Logger log = LogManager.getLogger(ismNoaelGeneralCoral.class);

    private static final long serialVersionUID = 1L;

    private static final String ModelData = "/data/model_noael_general_coral.xml";

    private CoralModel NoaelGeneral;

    public ismNoaelGeneralCoral() throws InitFailureException {
        super(ModelData);
        try {
            NoaelGeneral = new CoralGeneralNoael();
        } catch (Exception ex){
        ...
        this.ResultsName[0] = "NOAEL [log(mg/kg bw)]";
    ...
    protected short CalculateModel() {
        double Prediction;
        try {
            Prediction = NoaelGeneral.Predict(this.CurMolecule.GetSMILES());
        } catch (Exception ex) {
            return MODEL_ERROR;
        }
