...
public class Descriptors {
    ...
    private final ApicalCardioTox apicalCardioTox;
    private Path pathToExternalFolder;

    public Descriptors(ApicalCardioTox model) {
        apicalCardioTox = model;
    ...
    public boolean calculateDescriptors() throws IOException, InterruptedException, URISyntaxException {
        boolean isEnvSet = apicalCardioTox.CHECK_SETUP ? configureCondaEnv() : true;
        boolean result = false;
        if (isEnvSet) {
            log.info("Start to calculate descriptors");
            result = communication.executeCommandInCondaEnv("cddd", "cddd",
                    "--input "+apicalCardioTox.getInputTempFile(), " --output "+ apicalCardioTox.getDescriptorsTempFile(),
                    " --smiles_header smiles");
            File f = new File(apicalCardioTox.getInputTempFile());
            f.delete();
