...
public class ismAndrogenBindingComparaIRFMN extends InsilicoModel {
    private static final Logger log = LogManager.getLogger(ismAndrogenBindingComparaIRFMN.class);
    ...
    public ismAndrogenBindingComparaIRFMN() 
            throws InitFailureException {
        super(ModelData);
        
        // Set SA list
        SarpyFullAlertSet = AlertEncoding.MergeAlertIds((new SAAndrogenBindComparaIRFMN()).getAlerts());
        ...
        this.ResultsName[0] = "Predicted AR binding activity";
        this.ResultsName[1] = "No. alerts for binding activity";
        this.ResultsName[2] = "No. alerts for non-binding activity";
    ...
    public ArrayList<Integer> GetRequiredAlertBlocks() {
        ...
        blocks.add(InsilicoConstants.SA_BLOCK_ANDROGEN_BIND_COMPARA_SARPY);
        return blocks;
    }    
    ...
    protected short CalculateModel() {
        ...
        int nAllActNonInf = 0;
        int nAllInActInf = 0;
        int nAllInActNonInf = 0;
        int nAct6 = 0;

        for (Alert a : CurMolecule.GetAlerts().getSAList())
            if (AlertEncoding.ContainsAlert(SarpyFullAlertSet, a.getId())) {
                if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_AR_COMPARA_SARPY_ALL15_ACT_INF)) nAllActInf++;
                if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_AR_COMPARA_SARPY_ALL15_ACT_NONINF)) nAllActNonInf++;
                if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_AR_COMPARA_SARPY_ALL15_INACT_INF)) nAllInActInf++;
                if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_AR_COMPARA_SARPY_ALL15_INACT_NONINF)) nAllInActNonInf++;
                if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_AR_COMPARA_SARPY_ACT6)) nAct6++;
            }
        
        
        // Screening of rules from All15 set
        if (nAllActInf > 0) {
            MainResult = 1;
        } else {
            if (nAllActNonInf > 0) {
                if (nAllInActInf == 0) {
                    ...
                    if (nAllInActInf + nAllInActNonInf > 0) {
                        MainResult = 0;
                    } else {
                        MainResult = -1;
                    }
        ...
        // Screening of rules from Act6 set 
        if (MainResult == -1) {
            if (nAct6 > 0)
                MainResult = 1;
            else
        ...
        try {
            Res[0] = this.GetTrainingSet().getClassLabel(MainResult); // AR binding classification
        } catch (Throwable ex) {
            log.warn("Unable to find label for AR value " + MainResult);
            Res[0] = Integer.toString(MainResult);
        }
        Res[1] = Integer.toString(nAllActInf + nAllActNonInf + nAct6);
        Res[2] = Integer.toString(nAllInActInf + nAllInActNonInf);
