...
public class mainScriptPersistenceSoilQuantIRFMN {
    private static final Logger log = LogManager.getLogger(mainScriptPersistenceSoilQuantIRFMN.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismPersistenceSoilQuantitativeIrfmn();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-PersistanceSoilQuantitativeIRFMN\\src\\main\\resources\\data\\ts_soil_quantitative_irfmn.dat");
        try {
