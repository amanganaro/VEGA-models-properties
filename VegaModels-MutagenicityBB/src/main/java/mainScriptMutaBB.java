...
public class mainScriptMutaBB {
    private static final Logger log = LogManager.getLogger(mainScriptMutaBB.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismMutagenicityBB();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-MutagenicityBB\\src\\main\\resources\\data\\ts_muta_bb.dat");
        try {
