        ...
        try {
            for (int i=0; i<SMARTSFragments.length; i++)  {
                if (SMARTSFragments[i].isEmpty())
                    ...
                    SA[i] = SmartsPattern.create(SMARTSFragments[i]);
            }
        ...
        for (int i=0; i<nRings; i++) {
            ...
            if (IsBenzene(curRing)) {
                ...
                for (int k=0; k<curRing.getAtomCount(); k++) {
                    ...
                    for (IAtom iAtom : ConnAt) {
                        if (curRing.contains(iAtom))
                            continue;
                        if (iAtom.getAtomicNumber() == 9) {
                            nHalo++;
                            continue;
                        }
                        if (iAtom.getAtomicNumber() == 17) {
                            nHalo++;
                            continue;
                        }
                        if (iAtom.getAtomicNumber() == 35) {
                            nHalo++;
                            continue;
                        }
                        if (iAtom.getAtomicNumber() == 53) {
                            nHalo++;
                            continue;
                        }
                        if (iAtom.getAtomicNumber() == 6) {
                            // Checks if biphenyl
                            if (singleRings.contains(iAtom)) {
                                IRingSet buf = singleRings.getRings(iAtom);
