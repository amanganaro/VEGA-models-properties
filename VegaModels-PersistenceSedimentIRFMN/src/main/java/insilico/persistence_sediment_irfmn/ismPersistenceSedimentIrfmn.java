package insilico.persistence_sediment_irfmn;
...
import insilico.core.alerts.builders.SAPersistenceSediment;
...
import static insilico.core.constant.InsilicoConstants.KEY_ALERT_PERS_SEDIMENT_NP;
import static insilico.core.constant.InsilicoConstants.KEY_ALERT_PERS_SEDIMENT_VP;
...
public class ismPersistenceSedimentIrfmn extends InsilicoModel {
    private static final Logger log = LogManager.getLogger(ismPersistenceSedimentIrfmn.class);

    private static final long serialVersionUID = 1L;
    
    private static final String ModelData = "/data/model_pers_sed_irfmn.xml";
    ...
    public ismPersistenceSedimentIrfmn() 
            throws InitFailureException {
        super(ModelData);
        
        // Build SA list
        PersistenceAlertSet = AlertEncoding.MergeAlertIds(new SAPersistenceSediment().getAlerts());
        ...
        this.ResultsName[0] = "Predicted persistence in sediment";
        ...
        this.ADItemsName[3] = new ADIndexAlertsPersistenceSediment().GetIndexName();
    ...
    public ArrayList<Integer> GetRequiredAlertBlocks() {
        ...
        blocks.add(InsilicoConstants.SA_BLOCK_PERSISTENCE_SEDIMENT_IRFMN);
        return blocks;
    }    
            ...
            try {
                ...
                if (FoundSAs.size() > 0) {
                    ...
                    for (Alert CurAlert : FoundSAs.getSAList()) {
                        if (CurAlert.getBoolProperty(KEY_ALERT_PERS_SEDIMENT_VP))
                            nVP++;
                        if (CurAlert.getBoolProperty(KEY_ALERT_PERS_SEDIMENT_NP)) 
                            nNP++;
                    }
                    if ((nVP > 0)&&(nNP == 0)) 
    ...
    protected short CalculateAD() {
        ...
        try {
            ADIndexAlertsPersistenceSediment adalerts = new ADIndexAlertsPersistenceSediment();
        ...
        double saContribution = CurOutput.getADIndex(ADIndexAlertsPersistenceSediment.class).GetIndexValue();
