...
public class mainScriptPersistenceAirCoral {
    private static final Logger log = LogManager.getLogger(mainScriptPersistenceAirCoral.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismPersistenceAirCoral();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-PersistenceAirCoral\\src\\main\\resources\\data\\ts_pers_air_coral.dat");
        try {
