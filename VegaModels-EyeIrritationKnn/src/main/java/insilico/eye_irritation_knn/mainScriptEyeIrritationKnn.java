...
public class mainScriptEyeIrritationKnn {

    private static final Logger log = LogManager.getLogger(mainScriptEyeIrritationKnn.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismEyeIrritationKnn();
        model.setSkipADandTSLoading(false);
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-EyeIrritationSarpy\\src\\main\\resources\\data\\ts_eye_irritation_knn.dat");
        try {
