...
import insilico.core.molecule.conversion.custom.CustomCanonicalLabeler;
...
public class CustomSmilesWriterWithRings
{
...
//	private CanonicalLabeler canLabler = new CanonicalLabeler();
	private CustomCanonicalLabeler canLabler = new CustomCanonicalLabeler();
	 ...
	 */
	public CustomSmilesWriterWithRings() {}
	 ...
	 */
	public CustomSmilesWriterWithRings setRings(IRingSet rings)
	{
	  this.rings = rings;
	  return this;
	}
    ...
    public synchronized String createSMILES(IAtomContainer molecule) throws CDKException
	{
            ...
            // note: some exception could be raised by the called function
	}
	...
	{
		IMoleculeSet moleculeSet = (IMoleculeSet) ConnectivityChecker.partitionIntoMolecules(molecule);
		if (moleculeSet.getMoleculeCount() > 1)
		{
			...
			for (int i = 0; i < moleculeSet.getAtomContainerCount(); i++)
			{
				...
				fullSMILES.append(createSMILESWithoutCheckForMultipleMolecules(
						(IAtomContainer) molPart, chiral, doubleBondConfiguration));
...
//	@TestMethod("testCreateSMILESWithoutCheckForMultipleMolecules_withDetectAromaticity,testCreateSMILESWithoutCheckForMultipleMolecules_withoutDetectAromaticity")
	public synchronized String createSMILESWithoutCheckForMultipleMolecules(IAtomContainer molecule, boolean chiral, boolean doubleBondConfiguration[]) throws CDKException
	{
		...
		canLabler.canonLabel(molecule);
                    ...
                    throw new CDKException("Unable to run canonical labeler for the SMILES (" + e.getMessage() + ")");
                }
                brokenBonds.clear();
		ringMarker = 0;
		IAtom start = null;
		for (int i = 0; i < molecule.getAtomCount(); i++)
		{
			...
			if ((Long) atom.getProperty("CanonicalLable") == 1)
			{
                ...
                // custom by AM
		// detect rings, to check in next steps wheter an atom
                // is inside any ring or not
                if(rings == null){
                    if (ringFinder == null)
                    {
                    ...
                    try {
                        ...
                        throw new CDKException("Unable to process rings");
                    }
                }
		...
		if (container.getBond(atom, parent) != null)
		{
			if (container.getBond(atom, parent).getOrder() == CDKConstants.BONDORDER_DOUBLE && (lengthAtom == 3 || (lengthAtom == 2 && atom.getSymbol().equals("N"))) && (lengthParent == 3 || (lengthParent == 2 && parent.getSymbol().equals("N"))))
			{
				...
				for (int i = 0; i < atoms.size(); i++)
				{
					...
					{
						one = atomi;
					} else if (atomi != parent)
					{
						two = atomi;
					}
				}
				String[] morgannumbers = MorganNumbersTools.getMorganNumbersWithElementSymbol(container);
				return (one != null && two == null && atom.getSymbol().equals("N") && Math.abs(BondTools.giveAngleBothMethods(parent, atom, one, true)) > Math.PI / 10) || (!atom.getSymbol().equals("N") && one != null && two != null && !morgannumbers[container.getAtomNumber(one)].equals(morgannumbers[container.getAtomNumber(two)]));
			}
		}
		return (false);
	}
...
//	}
	...
	private List getCanNeigh(final IAtom a, final IAtomContainer container)
	{
		...
		if (v.size() > 1)
		{
			v.sort((Comparator) (o1, o2) -> (int) ((Long) ((IAtom) o1).getProperty("CanonicalLable") - (Long) ((IAtom) o2).getProperty("CanonicalLable")));
		}
		return v;
	}
	...
	private void parseAtom(IAtom a, StringBuffer buffer, IAtomContainer container, boolean chiral, boolean[] doubleBondConfiguration, IAtom parent, List atomsInOrderOfSmiles, List currentChain, boolean useAromaticity)
	{
		...
		if (chiral && isStartOfDoubleBond(container, a, parent, doubleBondConfiguration))
		{
			buffer.append('/');
                ...
                if (rings.contains(a)) inRing = true;
                brackets = brackets | !inRing;
...
//                        buffer.append("[").append(a.getSymbol().toLowerCase()).append("H]");
                    ...
                    if (a.getSymbol().equals("N") && nH == 1 && container.getConnectedAtomsList(a).size() != 3) {
                        buffer.append("[").append(a.getSymbol().toLowerCase()).append("H]");
...
// Added by AM
// if heteroatoms where a bracket is opened have hydrogens, they are explicitly set

// disabled for this kind of smarts/smiles

//                        if (brackets) 
//                            if (!symbol.equals("C"))
//                                if (a.getImplicitHydrogenCount() != null && a.getImplicitHydrogenCount() > 0) {
//                                    buffer.append("H");
//                                    if (a.getImplicitHydrogenCount() > 1) 
//                                        buffer.append(a.getImplicitHydrogenCount());
//                                }
                    }
...
//		try {
//			isotopeFactory = IsotopeFactory.class;
//		} catch (IOException e) {
//			e.printStackTrace();
//		}
//    }
