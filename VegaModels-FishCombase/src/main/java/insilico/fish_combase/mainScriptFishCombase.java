...
public class mainScriptFishCombase {
    private static final Logger log = LogManager.getLogger(mainScriptFishCombase.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismFishCombase();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-FishCombase\\src\\main\\resources\\data\\ts_fish_combase.dat");
        try {
