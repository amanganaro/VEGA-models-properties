...
public class mainScriptFishLC50 {
    private static final Logger log = LogManager.getLogger(mainScriptFishLC50.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismFishLC50();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-FishLC50\\src\\main\\resources\\data\\ts_fish_lc50.dat");
        try {
