package insilico.algae_combaseEC50;
...
public class ismAlgaeCombaseEC50 extends InsilicoModel {
    ...
    public ismAlgaeCombaseEC50()
            throws InitFailureException {
        super(ModelData);

        // Retrieve TS from algae classification model
        InsilicoModel algaeClass = new ismAlgaeCombaseClass();
        this.AlgaeClassTS = algaeClass.GetTrainingSet();


        // Define no. of descriptors
        this.DescriptorsSize = 15;
        this.DescriptorsNames = new String[DescriptorsSize];
        this.DescriptorsNames[0] = "B01[O-S]";
        this.DescriptorsNames[1] = "B02[N-Cl]";
        this.DescriptorsNames[2] = "B02[N-O]";
        this.DescriptorsNames[3] = "B03[O-Cl]";
        this.DescriptorsNames[4] = "B05[C-S]";
        this.DescriptorsNames[5] = "B06[N-Cl]";
        this.DescriptorsNames[6] = "B07[N-O]";
        this.DescriptorsNames[7] = "B09[C-C]";
        this.DescriptorsNames[8] = "F01[C-O]";
        this.DescriptorsNames[9] = "F05[O-O]";
        this.DescriptorsNames[10] = "MATS5m";
        this.DescriptorsNames[11] = "nR10";
        this.DescriptorsNames[12] = "X3A";
        this.DescriptorsNames[13] = "X3sol";
        this.DescriptorsNames[14] = "Toxicity Classification";
    ...
    protected short CalculateDescriptors(DescriptorsEngine DescEngine) {
        ...
        try {
            ...
            double[] algaeDesc = AlgaeDescEngine.GetDescriptorsForAlgaeQuantitativeModel();

            Descriptors = new double[DescriptorsSize];
            for (int i=0; i<DescriptorsSize-1; i++)
                Descriptors[i] = algaeDesc[i];


            // last descriptor is the output of the classification model
            // toxicity class is retrieved from exp value if available
            // otherwise it is just predicted with the original model
            Similarity sim = new Similarity();
            SimilarityDescriptorsBuilder simEng = new SimilarityDescriptorsBuilder();
            SimilarityDescriptors simDesc = simEng.Calculate(CurMolecule);
            Integer ExpToxClass = null;
            for (int i=0; i<AlgaeClassTS.getMoleculesSize(); i++) {
                double s = sim.CalculateExactMatches(simDesc,
                        AlgaeClassTS.getSimilarityDescriptor(i), CurMolecule.GetStructure(),
                        AlgaeClassTS.getSMILES(i));
                if (s == 1) {
                    ExpToxClass = (int)AlgaeClassTS.getExperimentalValue(i);
                    break;
                }
            }
            if (ExpToxClass != null) {
                Descriptors[DescriptorsSize-1] = ExpToxClass;
            } else {
                double[] algaeClassDesc = AlgaeDescEngine.GetDescriptorsForAlgaeClassificationModel();
                AlgaeClassificationANN AlgaeANN = new AlgaeClassificationANN();
                AlgaeANN.RunPrediction(algaeClassDesc);
                int ClassPrediction = AlgaeANN.ResultClass;
                Descriptors[DescriptorsSize-1] = ClassPrediction;
            }
    ...
    protected short CalculateModel() {
        ...
        AlgaeQuantitativeSVM svm = new AlgaeQuantitativeSVM();
        double Prediction = svm.RunPrediction(Descriptors);
