...
public class mainScriptSkinSensitizationConcert {

    private static final Logger log = LogManager.getLogger(mainScriptSkinSensitizationConcert.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismSkinSensitizationConcert();
        model.setSkipADandTSLoading(false);
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-SkinSensitizationConcert\\src\\main\\resources\\data\\ts_skin_sensitization_concert.dat");
        try {
