package insilico.watersolubility_tk.descriptors;
...
import insilico.watersolubility_tk.descriptors.weights.VertexDegree;
...
//                        List<IAtom> Path = PathTools.getShortestPath(m, at1, at2);
    ...
    private void CalculatePiPCandPW(InsilicoMolecule Mol) {
        try {
            MoleculePaths paths = new MoleculePaths(Mol);
            piPC8 = Math.log(1 + paths.Multiple_Path_Counts[7]);
            PW6 = paths.Pws[5];

        } catch (Exception e) {
            piPC8 = Descriptor.MISSING_VALUE;
            PW6 = Descriptor.MISSING_VALUE;
        }
    }
