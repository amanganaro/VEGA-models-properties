...
from models_dili_bayer.model_mtnn import MultiTaskNN
...
def make_predictions_with_ensemble(Xtest):
    ...
    hp_path = os.path.join(script_dir, 'models_dili_bayer', 'hyperparameters.json')
    ...
    for split_id in range(5): # ensemble with 5 models (one per split)
        ...
        script_dir = os.path.dirname(os.path.realpath(__file__))
        model_file = f'models_dili_bayer/model_mtnn_all_tasks_cddd_{split_id}.pth'
