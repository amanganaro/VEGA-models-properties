...
public class ismPersistenceWaterIrfmn extends InsilicoModel {
    private static final Logger log = LogManager.getLogger(ismPersistenceWaterIrfmn.class);

    private static final long serialVersionUID = 1L;
    
    private static final String ModelData = "/data/model_pers_water_irfmn.xml";
    ...
    public ismPersistenceWaterIrfmn() 
            throws InitFailureException {
        super(ModelData);
        
        // Build SA list
        PersistenceAlertSet = AlertEncoding.MergeAlertIds(new SAPersistenceWater().getAlerts());
        ...
        this.ResultsName[0] = "Predicted persistence in water";
    ...
    public ArrayList<Integer> GetRequiredAlertBlocks() {
        ...
        blocks.add(InsilicoConstants.SA_BLOCK_PERSISTENCE_WATER_IRFMN);
        return blocks;
    }    
    ...
    protected short CalculateAD() {
        ...
        try {
            ADIndexAlertsPersistenceWater adalerts = new ADIndexAlertsPersistenceWater();
        ...
        double saContribution = CurOutput.getADIndex(ADIndexAlertsPersistenceWater.class).GetIndexValue();
    ...
    protected void CalculateAssessment() {
        ...
        if (CurOutput.getMainResultValue() == Descriptor.MISSING_VALUE) {
            ...
            // Assessment is not built with default method, to add specific
            // warning for nP/P and P/vP classes
            
            String ResModified = CurOutput.getResults()[0];
            if (CurOutput.getMainResultValue() == 2) 
                ResModified = "nP/P (class of nP and P compounds with HL near the P threshold)"; // nP/P
            if (CurOutput.getMainResultValue() == 3) 
                ResModified = "P/vP (class of P and vP compounds with HL near the vP threshold)"; // P/vP
            
            // Check if ADI is missing
            if (CurOutput.getADI() == null) {

                CurOutput.setAssessment(CurOutput.getResults()[0]);
                CurOutput.setAssessmentVerbose(String.format(MessagesAD.ASSESS_LONG_NA, ResModified));
                ...
                switch (CurOutput.getADI().GetAssessmentClass()) {
                    case ADIndex.INDEX_LOW:
                        CurOutput.setAssessment(String.format(MessagesAD.ASSESS_SHORT_LOW, CurOutput.getResults()[0]));
                        CurOutput.setAssessmentVerbose(String.format(MessagesAD.ASSESS_LONG_LOW, ResModified, ADItemWarnings));
                        break;
                    case ADIndex.INDEX_MEDIUM:
                        CurOutput.setAssessment(String.format(MessagesAD.ASSESS_SHORT_MEDIUM, CurOutput.getResults()[0]));
                        CurOutput.setAssessmentVerbose(String.format(MessagesAD.ASSESS_LONG_MEDIUM, ResModified, ADItemWarnings));
                        break;
                    case ADIndex.INDEX_HIGH:
                        CurOutput.setAssessment(String.format(MessagesAD.ASSESS_SHORT_HIGH, CurOutput.getResults()[0]));
                        CurOutput.setAssessmentVerbose(String.format(MessagesAD.ASSESS_LONG_HIGH, ResModified));
            ...
            if (CurOutput.HasExperimental()) {
                String Exp = CurOutput.getExperimentalFormatted();
                String ExpModified = Exp;
                if (CurOutput.getExperimental()  == 2) 
                    ExpModified = "nP/P (class of nP and P compounds with HL near the P threshold)"; // nP/P
                if (CurOutput.getExperimental() == 3) 
                    ExpModified = "P/vP (class of P and vP compounds with HL near the vP threshold)"; // P/vP
                CurOutput.setAssessmentVerbose(String.format(MessagesAD.ASSESS_LONG_EXPERIMENTAL, ExpModified, CurOutput.getAssessment()));
                CurOutput.setAssessment(String.format(MessagesAD.ASSESS_SHORT_EXPERIMENTAL, Exp));
            }            
