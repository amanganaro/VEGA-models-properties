...
public class mainScriptSkinIrritationSarpy {

    private static final Logger log = LogManager.getLogger(mainScriptSkinIrritationSarpy.class);

    public static void main(String[] args) throws Exception {

        InsilicoModel model = new ismSkinIrritationSarpy();
//        model.setSkipADandTSLoading(false);
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-SkinIrritationSarpy\\src\\main\\resources\\data\\ts_skin_irritation_sarpy.dat");
        try {
