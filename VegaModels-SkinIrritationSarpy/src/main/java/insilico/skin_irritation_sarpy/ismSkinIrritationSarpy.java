...
import insilico.core.alerts.builders.SASkinIrritationConcert;
...
public class ismSkinIrritationSarpy extends InsilicoModel {
    ...
    private static final Logger log = LogManager.getLogger(ismSkinIrritationSarpy.class);
    ...
    public ismSkinIrritationSarpy() throws InitFailureException {
        super(ModelData);
        // Set SA list
        SarpyFullAlertSet = AlertEncoding.MergeAlertIds((new SASkinIrritationConcert()).getAlerts());
        ...
        this.ResultsName[0] = "Predicted Skin irritation activity";
        this.ResultsName[1] = "No. alerts for irritation activity";
    ...
    public ArrayList<Integer> GetRequiredAlertBlocks() {
        ...
        blocks.add(InsilicoConstants.SA_BLOCK_SKIN_IRRITATION_CONCERT_B3);
        return blocks;
    }
    ...
    protected short CalculateModel() {
        ...
        for (Alert a : CurMolecule.GetAlerts().getSAList())
            if (AlertEncoding.ContainsAlert(SarpyFullAlertSet, a.getId())) {
                if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_SARPY_STATS_INF)){
                    if (a.getBoolProperty(InsilicoConstants.KEY_ALERT_SKIN_IRR))
                        nIrritating++;
                    else
                        nNonIrritating++;
                }
        ...
        try {
            ...
            log.warn("Unable to find label for skin-irritation value " + MainResult);
