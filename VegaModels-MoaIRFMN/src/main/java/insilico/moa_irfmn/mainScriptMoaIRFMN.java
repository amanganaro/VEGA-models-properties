...
public class mainScriptMoaIRFMN {

    private static final Logger log = LogManager.getLogger(mainScriptMoaIRFMN.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismMoaIrfmn();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-MoaIRFMN\\src\\main\\resources\\data\\ts_moa_irfmn.dat");
        try {
