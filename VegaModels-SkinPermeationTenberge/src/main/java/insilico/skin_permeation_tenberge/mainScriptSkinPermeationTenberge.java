...
public class mainScriptSkinPermeationTenberge {
    private static final Logger log = LogManager.getLogger(mainScriptSkinPermeationTenberge.class);

    public static void main(String[] args) throws Exception {
        InsilicoModel model = new ismSkinPermeationTenBerge();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-SkinPermeationTenberge\\src\\main\\resources\\data\\ts_skin_perm_tenberge.dat");
        try {
