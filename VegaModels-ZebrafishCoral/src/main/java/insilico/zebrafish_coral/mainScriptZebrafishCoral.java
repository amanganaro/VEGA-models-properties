...
public class mainScriptZebrafishCoral {

    private static final Logger log = LogManager.getLogger(mainScriptZebrafishCoral.class);

    
    public static void main(String[] args) throws Exception {
        InsilicoModel model = new ismZebrafishCoral();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-ZebrafishCoral\\src\\main\\resources\\data\\ts_zebrafish_coral.dat");
        try {
