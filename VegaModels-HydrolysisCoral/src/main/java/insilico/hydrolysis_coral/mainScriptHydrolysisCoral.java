...
public class mainScriptHydrolysisCoral {

    private static final Logger log = LogManager.getLogger(mainScriptHydrolysisCoral.class);


    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismHydrolysisCoral();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-HydrolysisCoral\\src\\main\\resources\\data\\ts_hydrolysis_coral.dat");
        try {
