 ...
 */
public class GhoseCrippenWeightsFixed {
    ...
    /**
     * Provides the molar regfractivity contribution for an atom centered fragment
     * 
     * @param FragmentIndex index of the ACF
    ...
    private static void GetHydrophobitictyPrivate(IAtom at, IAtomContainer CurMol, int[] FragAtomId, double[][] ConnAugMatrix) {
        ...
        for (int j=0; j<nSK; j++) {
            if (j==At) 
                continue;
            if ( (ConnAugMatrix[At][j]>0) && (ConnAugMatrix[j][j] != 1) ) {
                VD++;
        ...
        try {
            ...
            int nH_impl = CurMol.getAtom(At).getImplicitHydrogenCount();
            
            int nH_expl = 0;
            for (int j=0; j<nSK; j++) {
                if (j==At) continue;
                if (ConnAugMatrix[j][At] == 1)
                    if (ConnAugMatrix[j][j] == 1)
                        nH_expl++;
            }
            
            nH = nH_expl > 0 ? nH_expl : nH_impl;
        ...
        if (ConnAugMatrix[At][At] == 6) {
            ...
            for (int j=0; j<nSK; j++) {
                if (j==At) continue;
                if ( (ConnAugMatrix[j][At]>0) && (ConnAugMatrix[j][j] != 1) ) {
                    c_VD++;
