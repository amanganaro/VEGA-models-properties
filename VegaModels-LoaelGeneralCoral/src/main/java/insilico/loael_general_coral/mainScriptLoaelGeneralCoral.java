...
public class mainScriptLoaelGeneralCoral {

    private static final Logger log = LogManager.getLogger(mainScriptLoaelGeneralCoral.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismLoaelGeneralCoral();
        model.setSkipADandTSLoading(false);
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-LoaelGeneralCoral\\src\\main\\resources\\data\\ts_loael_general_coral.dat");
        try {
