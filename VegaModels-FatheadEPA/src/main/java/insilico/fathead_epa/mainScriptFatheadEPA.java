...
public class mainScriptFatheadEPA {

    private static final Logger log = LogManager.getLogger(mainScriptFatheadEPA.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismFatheadEPA();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-FatheadEPA\\src\\main\\resources\\data\\ts_fathead_epa.dat");
        try {
