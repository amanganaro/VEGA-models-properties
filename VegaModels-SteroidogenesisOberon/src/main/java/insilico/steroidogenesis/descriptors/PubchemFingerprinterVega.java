...
public class PubchemFingerprinterVega extends AbstractFingerprinter implements IFingerprinter {
    ...
    private static Map<String, SmartsPattern> cache = new HashMap<>();

    public PubchemFingerprinterVega(IChemObjectBuilder builder) {
    ...
    public static class CountRings {
        ...
        private boolean isRingSaturated(IAtomContainer ring) {
            ...
            // fixed - padel seems to consider the ring saturated even if one bond is aromatic (condensed rings)
...
//                        || ringBond.getFlag(CDKConstants.SINGLE_OR_DOUBLE)) return false;
        ...
        public int countAromaticNitrogenContainingRing(int size) {
            int c = 0;
            for (IAtomContainer ring : ringSet.atomContainers()) {
                if (ring.getAtomCount() == size && isAromaticRing(ring)
                        && countNitrogenInRing(ring) > 0) ++c;
            }
            return c;
        }
    ...
    public static class CountSubstructures {
    ...
    public static BitSet decode(String enc) {
        ...
        PubchemFingerprinterVega pc = new PubchemFingerprinterVega(null);
    ...
    private static void countElements(byte[] fp, IAtomContainer mol) {
        int b;
        PubchemFingerprinterVega.CountElements ce = new PubchemFingerprinterVega.CountElements(mol);
    ...
    private static void countRings(byte[] fp, IAtomContainer mol) {
        PubchemFingerprinterVega.CountRings cr = new PubchemFingerprinterVega.CountRings(mol);
        int b;
    ...
    private void countSubstructures(byte[] fp, IAtomContainer mol) throws CDKException {
        PubchemFingerprinterVega.CountSubstructures cs = new PubchemFingerprinterVega.CountSubstructures(mol);
        int b;
