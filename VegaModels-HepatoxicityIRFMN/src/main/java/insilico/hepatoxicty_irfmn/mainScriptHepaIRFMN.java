...
public class mainScriptHepaIRFMN {
    private static final Logger log = LogManager.getLogger(mainScriptHepaIRFMN.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismHepatotoxicityIrfmn();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-HepatoxicityIRFMN\\src\\main\\resources\\data\\ts_hepa_irfmn.dat");
        try {
