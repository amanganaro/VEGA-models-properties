//import insilico.core.descriptor.blocks.old.KmFactor;
...
//        DescriptorBlock descriptorBlock = new KmFactor();
//        descriptorBlock.Calculate(SmilesMolecule.Convert("O=S(=O)(N)c1cc2c(cc1C(F)(F)F)NC(NS2(=O)(=O))Cc3ccccc3"));


        InsilicoModel model = new ismKmArnot();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-KmArnot\\src\\main\\resources\\data\\ts_km_arnot.dat");
        try {
