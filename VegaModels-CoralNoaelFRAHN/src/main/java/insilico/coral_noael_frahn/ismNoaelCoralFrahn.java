package insilico.coral_noael_frahn;

import insilico.coral.CoralModel;
import insilico.coral.models.noael.CoralNOAEL;
...
public class ismNoaelCoralFrahn extends InsilicoModel {

    private static final long serialVersionUID = 1L;

    private static final String ModelData = "/data/model_coral_noael_frahn.xml";

    private CoralModel NoaelFrhan;

    public ismNoaelCoralFrahn() throws InitFailureException {
        super(ModelData);

        // Init Coral model
        try {
            NoaelFrhan = new CoralNOAEL();
        } catch (Exception ex) {
...
//        this.ResultsName[2] = "Experimental NOAEL [mg/kg]";
    ...
    protected short CalculateModel() {
        double Prediction;
        try {
            Prediction = NoaelFrhan.Predict(this.CurMolecule.GetSMILES());
        } catch (Exception ex) {
            return MODEL_ERROR;
        }
