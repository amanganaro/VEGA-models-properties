...
public class mainScriptDaphniaEPA {
    private static final Logger log = LogManager.getLogger(mainScriptDaphniaEPA.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismDaphniaEPA();
//        ModelsDeployment.BuildDataset(model, "out_ts");
//        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
//        File destinationFile = new File("VegaModels-DaphniaEPA\\src\\main\\resources\\data\\ts_daphnia_epa.dat");
//        try {
