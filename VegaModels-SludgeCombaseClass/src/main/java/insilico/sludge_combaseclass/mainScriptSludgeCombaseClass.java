...
public class mainScriptSludgeCombaseClass {


    private static final Logger log = LogManager.getLogger(mainScriptSludgeCombaseClass.class);

    public static void main(String[] args) throws Exception {
        InsilicoModel model = new ismSludgeCombaseClass();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-SludgeCombaseClass\\src\\main\\resources\\data\\ts_sludge_combaseClass.dat");
        try {
