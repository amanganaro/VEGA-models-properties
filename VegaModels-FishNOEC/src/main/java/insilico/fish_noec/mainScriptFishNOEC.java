...
public class mainScriptFishNOEC {
    private static final Logger log = LogManager.getLogger(mainScriptFishNOEC.class);

    public static void main(String[] args) throws Exception {


        InsilicoModel model = new ismFishNOEC();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-FishNOEC\\src\\main\\resources\\data\\ts_fish_noec.dat");
        try {
