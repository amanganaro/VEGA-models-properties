...
public class mainScriptNoaelCoralLiver {
    ...
    public static void main(String[] args) throws InitFailureException, GenericFailureException, MalformedURLException, FileNotFoundException {
        InsilicoModel model = new ismNoaelCoralLiver();
        ModelsDeployment.BuildDataset(model, "out_ts");
        File sourceFile = new File("out_ts/" + model.getInfo().getTrainingSetURL() + "/" + model.getInfo().getTrainingSetURL().split("/data/")[1]);
        File destinationFile = new File("VegaModels-NoaelLiver\\src\\main\\resources\\data\\ts_noael_liver_coral.dat");
        try {
